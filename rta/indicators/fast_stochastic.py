...
class FastStochastic(IndicatorBase):
  ...
  def calculate(self):
    pivot, s1, s2 = getattr(Indicators, self.func).__call__( self.series['high'], self.series['low'], self.series['close'], self.fastk_period, self.fastd_period, self.fastd_matype )
  ...
  def options(_cls, kwgs):
    defaults = dict({ 'fastk_period' : 5, 'fastd_period' : 3, 'fastd_matype' : 0 })
    return dict( defaults.items() + kwgs.items() )

[ IndicatorFactory.register(x, FastStochastic) for x in [ 'STOCHF' ] ]
