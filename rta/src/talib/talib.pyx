...
def ACOS( np.ndarray[np.float_t, ndim=1] real ):
    ...
    cdef np.ndarray[np.float_t, ndim=1] outreal = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def AROON( np.ndarray[np.float_t, ndim=1] high , np.ndarray[np.float_t, ndim=1] low , timeperiod=-2**31 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outaroondown = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outaroonup = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def BBANDS( np.ndarray[np.float_t, ndim=1] real , timeperiod=-2**31 , nbdevup=-4e37 , nbdevdn=-4e37 , matype=0 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outrealupperband = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outrealmiddleband = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outreallowerband = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def CDL2CROWS( np.ndarray[np.float_t, ndim=1] open , np.ndarray[np.float_t, ndim=1] high , np.ndarray[np.float_t, ndim=1] low , np.ndarray[np.float_t, ndim=1] close ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.int_t, ndim=1] outinteger = numpy.zeros(allocation, numpy.int)
    TA_Initialize()
...
def HT_PHASOR( np.ndarray[np.float_t, ndim=1] real ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outinphase = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outquadrature = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def HT_SINE( np.ndarray[np.float_t, ndim=1] real ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outsine = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outleadsine = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def MACD( np.ndarray[np.float_t, ndim=1] real , fastperiod=-2**31 , slowperiod=-2**31 , signalperiod=-2**31 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outmacd = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outmacdsignal = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outmacdhist = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def MAMA( np.ndarray[np.float_t, ndim=1] real , fastlimit=-4e37 , slowlimit=-4e37 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outmama = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outfama = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def MINMAX( np.ndarray[np.float_t, ndim=1] real , timeperiod=-2**31 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outmin = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outmax = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def MINMAXINDEX( np.ndarray[np.float_t, ndim=1] real , timeperiod=-2**31 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.int_t, ndim=1] outminidx = numpy.zeros(allocation, numpy.int)
    cdef np.ndarray[np.int_t, ndim=1] outmaxidx = numpy.zeros(allocation, numpy.int)
    TA_Initialize()
...
def STOCH( np.ndarray[np.float_t, ndim=1] high , np.ndarray[np.float_t, ndim=1] low , np.ndarray[np.float_t, ndim=1] close , fastk_period=-2**31 , slowk_period=-2**31 , slowk_matype=0 , slowd_period=-2**31 , slowd_matype=0 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outslowk = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outslowd = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
...
def STOCHF( np.ndarray[np.float_t, ndim=1] high , np.ndarray[np.float_t, ndim=1] low , np.ndarray[np.float_t, ndim=1] close , fastk_period=-2**31 , fastd_period=-2**31 , fastd_matype=0 ):
    ...
    cdef int outbegidx
    cdef int outnbelement
    cdef np.ndarray[np.float_t, ndim=1] outfastk = numpy.zeros(allocation, numpy.float)
    cdef np.ndarray[np.float_t, ndim=1] outfastd = numpy.zeros(allocation, numpy.float)
    TA_Initialize()
